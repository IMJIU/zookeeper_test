package com.curator.t03_distribute_lock;
...
public class LockUtil2 {
	...
	private static CountDownLatch shardLocklatch = new CountDownLatch(1);
	...
	public static synchronized void init(String connectString) {
		...
		client = CuratorFrameworkFactory.builder().connectString(connectString).sessionTimeoutMs(10000).retryPolicy(retryPolicy).namespace("LockService").build();
		client.start();

		// 创建锁目录
		try {
			if (client.checkExists().forPath("/ExclusiveLockDemo") == null) {
				client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).withACL(Ids.OPEN_ACL_UNSAFE).forPath("/ExclusiveLockDemo");
			}
			// 创建锁监听
			addChildWatcher("/ExclusiveLockDemo");
			if (client.checkExists().forPath("/ShardLockDemo") == null) {
				client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).withACL(Ids.OPEN_ACL_UNSAFE).forPath("/ShardLockDemo");
			}
		} catch (Exception e) {
	...
	public static synchronized void getExclusiveLock() {
		while (true) {
			try {
				client.create().creatingParentsIfNeeded().withMode(CreateMode.EPHEMERAL).withACL(Ids.OPEN_ACL_UNSAFE).forPath("/ExclusiveLockDemo/lock");
				logger.info("成功获取到锁");
	...
	/**
	 * 
	 * @param type 0为读锁，1为写锁
	 * @param identity 获取当前锁的所有者
	 */
	public static boolean getShardLock(int type, String identity) {
		...
		try {
			// if (client.checkExists().forPath("/ShardLockDemo/" + nodeName) ==
			// null)
			selfNodeName = client.create().creatingParentsIfNeeded().withMode(CreateMode.EPHEMERAL_SEQUENTIAL).withACL(Ids.OPEN_ACL_UNSAFE).forPath("/ShardLockDemo/" + nodeName);
