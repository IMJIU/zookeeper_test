...
public class LockDemo {  
	 ...
	 * @描述：第一种锁: 共享锁
	 * @return void
	 * @exception @createTime：2016年5月19日
	 * @author: songqinghu
	 * @throws Exception
	 */
	...
	private static void readWriterLock(InterProcessLock lock) {
		...
		try {
			if (lock.acquire(20, TimeUnit.MILLISECONDS)) {
				// System.err.println(Thread.currentThread().getName() + " 等待超时
				// 无法获取到锁");
				// 执行任务 --读取 或者写入
	...
	/**
	 * 共享信号量 设置总的数量 -->分布式情况下的最大并行数量 按照请求顺序进行 执行权的分配 可以设置超时 不执行 也可以设置 直到获取执行权 执行
	 */
	...
	private static CuratorFramework getClient() {
		if (client == null) {
			...
			CuratorFramework clientinit = CuratorFrameworkFactory.builder().aclProvider(aclProvider).authorization(scheme, auth).connectionTimeoutMs(connectionTimeoutMs)
					.connectString(connectString).defaultData(defaultData).maxCloseWaitMs(maxCloseWaitMs).namespace(namespace).retryPolicy(retryPolicy).build();
